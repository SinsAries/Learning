...
void solve(void) {
	...
	F0R(i, n) 
		for (auto &x : dp[i]) {
			if(dp[i + 1].find(x.f) == dp[i + 1].end()) 
				dp[i + 1][x.f] = x.s;
			else 
				dp[i + 1][x.f] = min(dp[i + 1][x.f], x.s);
			ll nex_gcd = __gcd(x.f, l[i]);
			if(dp[i + 1].find(nex_gcd) == dp[i + 1].end()) 
				dp[i + 1][nex_gcd] = x.s + c[i];
			else
				dp[i + 1][nex_gcd] = min(dp[i + 1][nex_gcd], x.s + c[i]);
		}
	
	if(dp[n].find(1LL) == dp[n].end())
		cout << "-1";
	else
		cout << dp[n][1];
}
